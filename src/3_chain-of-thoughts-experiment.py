"""
Chain-of-Thought Stance Prediction Experiment
==============================================

A single script that combines all Chain-of-Thought experiment variants into one.
Use command-line arguments to control the experiment settings.

All runs use Chain-of-Thought prompt templates by default.

Usage Examples:
    # Basic run: no transcripts, most similar examples, sequential
    python chain-of-thoughts-experiment.py --num_workers 1

    # Run with summarized transcripts
    python chain-of-thoughts-experiment.py --model gemma3 --transcripts Summarized

    # Run with reasoning in most-similar examples, parallel
    python chain-of-thoughts-experiment.py --include_reasoning

    # Custom k values and targets
    python chain-of-thoughts-experiment.py --model llama3-sdsc --k_values 1 5 --targets debt eps
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run Chain-of-Thought stance prediction experiments with various LLMs.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python chain-of-thoughts-experiment.py
  python chain-of-thoughts-experiment.py --model gemma3 --transcripts Summarized
  python chain-of-thoughts-experiment.py --include_reasoning
  python chain-of-thoughts-experiment.py --model llama3-sdsc --k_values 1 5 --num_workers 8
        """,
    )
    ...
    main_group.add_argument(
        ...
        help=(
            ...
            "DeepSeek-R1-Distill-Qwen-32B, mistral-small:24b, "
            "phi4-reasoning:14b, qwen3:32b. "
            "Default: gpt-4.1-mini-2025-04-14."
        ),
    )
    ...
    main_group.add_argument(
        ...
        action="store_true",
        help=(
            "Include LLM reasoning alongside each most-similar few-shot example. "
            "NOTE: This only takes effect when --few_shot_sampling is 'most_similar'. "
            "Random sampling always uses the standard format. Default: off"
        ),
    )
...
def build_save_directory(args):
    ...
    For example:
        few-shot-chain-of-thought-no-transcript-most-similar-examples
    """
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    base = os.path.join(project_root, 'experiments', 'results', 'chain-of-thought')

    name_parts = ["cot"]

    if args.transcripts == "No":
        name_parts.append("no-transcript")
    elif args.transcripts == "Full":
        name_parts.append("transcripts")
    elif args.transcripts == "Summarized":
        name_parts.append("transcripts-summarized")
    ...
    if args.include_reasoning:
        name_parts.append("reasoning-from-chatGPT")
...
def print_config():
    ...
    print("=" * 60)
    print("  Chain-of-Thought Stance Prediction Experiment")
    print("=" * 60)
...
def get_prompt(data_type, text, target, quarter, year, company, filename, few_shot_examples):
    """
    Build the full prompt by combining a Chain-of-Thought template,
    optional context, and few-shot examples.
    """
    # Always use Chain-of-Thought prompt templates
    prompt_paths = {
...
def read_random_few_shot_examples(text, filepath, k, stance_column):
    ...
    Examples are grouped by class. This function uses the same format regardless
    of whether --include_reasoning is set (reasoning only affects most_similar sampling).
    ...
    # Collect examples grouped by class
...
def read_closest_few_shot_examples(text, filepath, k, stance_column):
    ...
    When --include_reasoning is enabled, each example also shows the LLM's
    reasoning text and uses a different display format.
    ...
    if ARGS.include_reasoning:
        # ---- With reasoning: includes LLM_Reason alongside each example ----
        ...
        for cat in ["Positive", "Negative", "Neutral"]:
            ...
            exs = examples[cat]
            if exs:
                # Only use the text part for computing similarity
                example_texts = [ex[0] for ex in exs]
                while True:
                    try:
                        embs = client_embed.get_embeddings(example_texts)
                        break
                    except Exception as e:
        ...
        # ---- Without reasoning: standard grouped-by-class format ----
        examples = {cat: [] for cat in ["Positive", "Negative", "Neutral"]}
        for _, row in df.iterrows():
            stance = row[stance_column]
            if stance in examples:
